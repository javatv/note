...
可以看到，在 Worker 的构造方法可以知道，其中的 thread 属性就是通过this去创建的，所以线程池核心线程的创建主要是 run 方法中的 **runWorker** 方法：
...
在 `allowCoreThreadTimeOut`默认情况下，也就是当线程池中的总线程数大于核心线程数时，`timed = true`，此时看如下代码：
